  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-black/95 border-b border-white/10 backdrop-blur-md' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between h-24 md:h-28">
        ...
        <ul className="hidden md:flex items-center gap-14 lg:gap-18">
          {navItems.map((item) => (
            <li key={item.href}>
              <a
                href={item.href}
                onClick={(e) => handleNavClick(e, item.href)}
                className="text-[#aaa] hover:text-white text-sm tracking-[0.12em] uppercase transition-colors duration-300 cursor-pointer whitespace-nowrap relative group px-3 py-1"
              >
                {item.label}
